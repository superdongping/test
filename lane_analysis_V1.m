function lane_analysis_V1()
    % Load the image (prompt the user to choose the file)
    [file, path] = uigetfile({'*.png;*.jpg;*.jpeg', 'Image Files (*.png, *.jpg, *.jpeg)'}, 'Select an Image');
    if isequal(file, 0)
        disp('User canceled image selection');
        return;
    end
    img = imread(fullfile(path, file));  % Read the selected image
    gray_img = rgb2gray(img);  % Convert to grayscale

    % Invert the intensity: black (0) becomes highest (255) and white (255) becomes lowest (0)
    inverted_img = 255 - gray_img;

    % Display the inverted image
    imshow(inverted_img);
    title('Select the first lane manually (Inverted Intensity)');
    ...
    while continue_drawing
        ...
        if strcmp(answer, 'Yes')
            % Display the inverted image and let the user click to define the top-left corner
            imshow(inverted_img);
            ...
            % Draw the new rectangle in real-time on the inverted image
            rectangle('Position', new_rect_position, 'EdgeColor', 'r', 'LineWidth', 2);
            hold off;  % Update the image to show the newly drawn rectangle

            % Crop the ROI for the current lane from the inverted image
            roi = imcrop(inverted_img, new_rect_position);
    ...
    writetable(signal_table, 'signal_profiles_inverted.xlsx');

    % Final display of all selected lanes on the inverted image
    imshow(inverted_img);
    title('Selected ROIs');
    hold on;
    ...
    title('Signal Intensity Profiles (Inverted)');
    xlabel('Position (pixels)');
    ylabel('Inverted Intensity');
    legend show;
